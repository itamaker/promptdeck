...
1. Create a GitHub repository named `promptdeck`.
...
```bash
./scripts/render-homebrew-formula.sh --owner itamaker --version v0.1.0 > /path/to/homebrew-tap/Formula/promptdeck.rb
```

Commit the rendered file to `https://github.com/itamaker/homebrew-tap` as `Formula/promptdeck.rb`, then users can install with:
